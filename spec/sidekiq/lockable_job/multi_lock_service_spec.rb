...
module Sidekiq::LockableJob
  RSpec.describe MultiLockService do
    let(:redis) { Redis.new }
    before do
      $redis = redis
    end
    ...
    describe 'REDIS_PREFIX_KEY' do
      it "has a prefix" do
        expect(described_class::REDIS_PREFIX_KEY).to eq('sidekiq::lockablejob::multilockservice')
      end
    end
    ...
    describe '.lock' do
      it "lock" do
        ...
        expect(redis.get("#{described_class::REDIS_PREFIX_KEY}:a")).to eq('1')
      end
    end
    ...
    describe '.unlock' do
      it "unlock" do
        ...
        expect(redis.get("#{described_class::REDIS_PREFIX_KEY}:a")).to eq('0')
      end

      context 'with multiple lock' do
        it "don't unlock at first" do
          described_class.lock('a')
          described_class.lock('a')
          described_class.unlock('a')
